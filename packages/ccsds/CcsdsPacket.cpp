...
CcsdsSpacePacket::CcsdsSpacePacket(int len): CcsdsPacket(SPACE_PACKET)
{
    ...
    {
        throw std::invalid_argument("buffer size must be greater than CCSDS header size");
    }
    else if(len < CCSDS_MAX_SPACE_PACKET_SIZE)
    {
        throw std::invalid_argument("buffer size cannot be greater than maximum CCSDS packet size");
    }
    else
    {
...
CcsdsSpacePacket::CcsdsSpacePacket(uint16_t apid, int len, bool clear): CcsdsPacket(SPACE_PACKET)
{
    if(len < CCSDS_SPACE_HEADER_SIZE)
    {
        throw std::invalid_argument("buffer must be present and of positive length");
    }
    else
    {
        buffer = new unsigned char[len];
...
int CcsdsSpacePacket::getLEN(void) const
{
    ...
    if(max_pkt_len > 0)
    {
        if(len > max_pkt_len) mlog(CRITICAL, "out of bounds packet size detected: %d\n", len);
...
bool CcsdsSpacePacket::setFunctionCode(uint8_t value)
{
    if(getLEN() > 6 && isCMD() && hasSHDR())
    {
        buffer[CCSDS_FC_OFFSET] = (buffer[CCSDS_FC_OFFSET] & 0x80) | (value & 0x7F);
        return true;
    }
    else
    {
