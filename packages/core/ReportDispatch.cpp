...
int ReportDispatch::luaCreate (lua_State* L)
{
    ...
    try
    {
        ...
        if(file_format == INVALID_FORMAT)
        {
            ...
            throw LuaException("parameter error");
        }
...
/******************************************************************************
 * PRIVATE METOHDS - REPORT DISAPTCH
 *******************************************************************************/
...
bool ReportDispatch::processRecord (RecordObject* record, okey_t key)
{
    ...
    try
    {
        ...
        if(!name || !value) throw AccessRecordException("received incomplete metric");
    }
