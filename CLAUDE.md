...
4. **Update the OpenAPI spec** — add the endpoint path to `sliderule-openapi-complete.yaml`:
   - Add a path entry under the appropriate section
   - For Parquet endpoints: add a response column schema in `components/schemas/`
...
3. **Update the OpenAPI spec** — update both the response column schema and any description text
...
4. **Remove the path and response schema** from `sliderule-openapi-complete.yaml`

### Validation

After any endpoint change, verify:
- `make selftest` passes (schema tests validate the registry)
- The OpenAPI YAML is valid: `python3 -c "import yaml; yaml.safe_load(open('sliderule-openapi-complete.yaml'))"`
...
- `sliderule-openapi-complete.yaml` — OpenAPI 3.1 spec (must stay in sync with code)
