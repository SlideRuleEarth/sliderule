...
def test_nominal(client):
    ...
    response = client.post('/metrics/issue_alarm', json=request)
...
def test_value_counts(client):
    ...
    response = client.get('/status/alarm_counts/status_code')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
    assert data["0"] == 2
    # value counts - version
    response = client.get('/status/alarm_counts/version')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
...
def test_list(client):
    ...
    response = client.post('/metrics/issue_alarm', json={
    ...
    response = client.get('/status/alarm_list?duration=31622400')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
    assert len(data) == 1
    assert data[0]['alarm_time'] == one_year_ago + ' 00:00:00'
    # list - 2 years (and a day)
    response = client.get('/status/alarm_list?duration=63158400')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
    assert len(data) == 2
    assert data[0]['alarm_time'] == one_year_ago + ' 00:00:00'
    ...
    response = client.get('/status/alarm_list?duration=94694400')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
    assert len(data) == 3
    assert data[0]['alarm_time'] == one_year_ago + ' 00:00:00'
    ...
    response = client.get(f'/status/alarm_list?t0=\'{three_years_ago}\'&t1=\'{one_year_ago}\'')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
    assert len(data) == 3
    assert data[0]['alarm_time'] == one_year_ago + ' 00:00:00'
    ...
    response = client.get(f'/status/alarm_list?t0=\'{two_years_ago}\'&t1=\'{one_year_ago}\'')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
    assert len(data) == 2
    assert data[0]['alarm_time'] == one_year_ago + ' 00:00:00'
    ...
    response = client.get(f'/status/alarm_list?t0=\'{one_year_ago}\'')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
    assert len(data) == 1
    assert data[0]['alarm_time'] == one_year_ago + ' 00:00:00'
    # list - between 2 years ago and before
    response = client.get(f'/status/alarm_list?t1=\'{two_years_ago}\'')
    data = json.loads(response.data.decode("utf-8"))
    assert response.status_code == 200
    assert len(data) == 2
