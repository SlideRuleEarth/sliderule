...
# VPC
# ---------------------------------------------------------------------------------------------------------------------
resource "aws_vpc" "static-website" {
...
resource "aws_subnet" "private" {
  count             = var.az_count
  cidr_block        = cidrsubnet(aws_vpc.static-website.cidr_block, 8, count.index)
...
resource "aws_subnet" "public" {
  count                   = var.az_count
  cidr_block              = cidrsubnet(aws_vpc.static-website.cidr_block, 8, var.az_count + count.index)
  availability_zone       = data.aws_availability_zones.available.names[count.index]
  vpc_id                  = aws_vpc.static-website.id
...
resource "aws_internet_gateway" "static-website-igw" {
  vpc_id = aws_vpc.static-website.id
  tags = {
...
resource "aws_route" "public-route" {
  route_table_id         = aws_vpc.static-website.main_route_table_id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.static-website-igw.id
}
...
resource "aws_eip" "eip" {
  count      = var.az_count
  vpc        = true
  depends_on = [aws_internet_gateway.static-website-igw]
  tags = {
