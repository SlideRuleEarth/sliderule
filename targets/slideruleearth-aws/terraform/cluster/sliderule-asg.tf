resource "aws_launch_template" "sliderule_template" {
  ...
  user_data = base64encode(<<-EOF
    ...
    aws s3 cp s3://sliderule/config/coastnet.so /plugin/coastnet.so
    aws s3 cp s3://sliderule/config/qtrees.so /plugin/qtrees.so
    aws s3 cp s3://sliderule/config/openoceanspp.so /plugin/openoceanspp.so
