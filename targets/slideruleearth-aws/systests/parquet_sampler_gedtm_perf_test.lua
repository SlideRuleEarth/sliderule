...
local _out_parquet   = "/data/gedtm/output/wrzesien_snow_samples.parquet"
...
local robj_gedtm = geo.raster(geo.parms({asset="gedtm-30meter", algorithm="NearestNeighbour", radius=0, catalog = contents, use_poi_time=false}))
runner.assert(robj_gedtm ~= nil)

print('\n--------------------------------------\nTest01: input/output parquet (x, y)\n--------------------------------------')
local starttime = time.latch();
parquet_sampler = arrow.sampler(core.parms({output={path=out_parquet, format="parquet"}}), in_parquet, outq_name, {["gedtm"] = robj_gedtm})
