...
If you haven't already, install the SlideRule Python Client using the [instructions](Install.md) provided.
...
1. Imports the `sliderule` module from the SlideRule Python package
2. Defines a polygon that captures the Grand Mesa region (note: sliderule also supports using geojson, shapefiles, and GeoDataFrames to define an area of interest)
3. Builds an API request parameter structure that specifies the region of interest, a specific track to process, a photon confidence level, and the surface fit algorithm
4. Issues the request to the SlideRule service to perform the specified processing on ATL03 and returns the results in a GeoDataFrame for further analysis
