...
@pytest.mark.network
class TestApi:
    def test_happy_case(self, domain, asset, organization, desired_nodes):
        sliderule.init(domain, organization=organization, desired_nodes=desired_nodes, bypass_dns=True)
        epoch_offset = h5coro.h5("ancillary_data/atlas_sdp_gps_epoch", ATL03_FILE1, asset)[0]
        assert epoch_offset == 1198800018.0

    def test_h5_types(self, domain, asset, organization, desired_nodes):
        sliderule.init(domain, organization=organization, desired_nodes=desired_nodes, bypass_dns=True)
        heights_64 = h5coro.h5("/gt1l/land_ice_segments/h_li", ATL06_FILE1, asset)
    ...
    def test_variable_length(self, domain, asset, organization, desired_nodes):
        sliderule.init(domain, organization=organization, desired_nodes=desired_nodes, bypass_dns=True)
        v = h5coro.h5("/gt1r/geolocation/segment_ph_cnt", ATL03_FILE1, asset)
        assert v[0] == 258 and v[1] == 256 and v[2] == 273

    def test_invalid_file(self, domain, asset, organization, desired_nodes):
        sliderule.init(domain, organization=organization, desired_nodes=desired_nodes, bypass_dns=True)
        v = h5coro.h5("/gt1r/geolocation/segment_ph_cnt", INVALID_FILE, asset)
        assert len(v) == 0
    ...
    def test_invalid_path(self, domain, asset, organization, desired_nodes):
        sliderule.init(domain, organization=organization, desired_nodes=desired_nodes, bypass_dns=True)
        v = h5coro.h5("/gt1r/invalid-path", ATL03_FILE1, asset)
        assert len(v) == 0
