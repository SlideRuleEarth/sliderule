...
class Test3DEP_10meter:
    ...
    def test_legacy_sample_with_slope_aspect(self, init):
        ...
        assert abs(gdf["3dep.slope"].iloc[0]  - 5.615220456398121) < sigma, f'slope = {gdf["3dep.slope"].iloc[0]}'
        assert abs(gdf["3dep.aspect"].iloc[0] - 68.08075892919926) < sigma, f'aspect = {gdf["3dep.aspect"].iloc[0]}'
