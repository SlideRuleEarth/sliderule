...
local timeout = parms["rqst-timeout"] or parms["timeout"] or gedi.RQST_TIMEOUT
local node_timeout = parms["node-timeout"] or parms["timeout"] or gedi.NODE_TIMEOUT
...
if parms[arrow.PARMS] then
    ...
    if output_parms:isparquet() then
        ...
        parquet_builder = arrow.parquet(output_parms, rspq, rsps_from_nodes, "gedi01b", "gedi01b.footprint", rqstid, "longitude", "latitude")
    end
end

-- Proxy Request --
local proxy = netsvc.proxy("gedi01b", resources, json.encode(parms), node_timeout, rsps_from_nodes, terminate_proxy_stream)
