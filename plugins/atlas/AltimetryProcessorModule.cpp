...
CommandableObject* AltimetryProcessorModule::createObject(CommandProcessor* cmd_proc, const char* name, int argc, char argv[][MAX_CMD_SIZE])
{
    ...
    {
        mlog(CRITICAL, "Invalid pce specified: %d\n", pce);
        return NULL;
    }
...
bool AltimetryProcessorModule::parseAltHist(List<CcsdsSpacePacket*>& segments, int numpkts)
{
    ...
    {
        mlog(ERROR, "Altimetric histogram packet with wrong number of segments: %d\n", numsegs);
        return false;
    }
...
//                else                                mlog(WARNING, "[%04X]: could not associate major frame data with science time tag data\n", mfc);
            }
            ...
            {
                mlog(ERROR, "to many bins in altimetric range window %d, max is %d\n", num_bins, max_bins);
                pkt_errors++;
                num_bins = max_bins;
            }
    ...
    if(!sigfound)
    {
        mlog(WARNING, "[%08lX]: could not find signal in altimetric histogram data\n", hist->getMajorFrameCounter());
    }
    ...
    if(mfdata_ptr != NULL)
    {
        ...
        if(numpkts == 1) // only works if not integrating
        {
            ...
            {
                mlog(ERROR, "[%08lX]: %s alimteric range window start did not match value reported by hardware, FSW: %.1lf, DFC: %.1lf\n", hist->getMajorFrameCounter(), AtlasHistogram::type2str(type), hist->getRangeWindowStart(), dfc_rws);
                pkt_errors++;
            }
            ...
            {
                mlog(ERROR, "[%08lX]: %s alimteric range window width did not match value reported by hardware, FSW: %.1lf, DFC: %.1lf\n", hist->getMajorFrameCounter(), AtlasHistogram::type2str(type), hist->getRangeWindowWidth(), dfc_rww);
                pkt_errors++;
            }
        }
    }
...
//            else                                mlog(WARNING, "[%04X]: could not associate major frame data with science time tag data\n", mfc);
    ...
    if(!sigfound)
    {
        mlog(WARNING, "[%08lX]: could not find signal in atmospheric histogram data\n", hist->getMajorFrameCounter());
    }
    ...
    if(mfdata_ptr != NULL)
    {
        ...
        if(numpkts == 1) // only works if not integrating
        {
            ...
            if(dfc_rws != hist->getRangeWindowStart())
            {
                mlog(ERROR, "[%08lX]: %s atmospheric range window start did not match value reported by hardware, FSW: %.1lf, DFC: %.1lf\n", hist->getMajorFrameCounter(), AtlasHistogram::type2str(type), hist->getRangeWindowStart(), dfc_rws);
                pkt_errors++;
            }
            ...
            if(dfc_rww != hist->getRangeWindowWidth())
            {
                mlog(ERROR, "[%08lX]: %s atmospheric range window width did not match value reported by hardware, FSW: %.1lf, DFC: %.1lf\n", hist->getMajorFrameCounter(), AtlasHistogram::type2str(type), hist->getRangeWindowWidth(), dfc_rww);
                pkt_errors++;
            }
        }
    }
