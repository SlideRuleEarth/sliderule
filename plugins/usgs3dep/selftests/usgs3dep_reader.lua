...
-- echo -108.1 39.1 0.0 | PROJ_DEBUG=2 PROJ_NETWORK=ON cs2cs -r -s epsg:7912 epsg:6342+5703
-- at the end of output there should be a statement:
-- Using https://cdn.proj.org/us_noaa_g2018u0.tif
-- 4332449.53      231917.92 16.53
--
-- The pixel coordinates can be compared to the debug messages generated by code if DEBUG is enabled in the script.
-- This is the only way to validate that the transform created by code generates the same pixels than cs2cs tool.

local expected_value = 2654.558105 -- first sample from the group of samples
