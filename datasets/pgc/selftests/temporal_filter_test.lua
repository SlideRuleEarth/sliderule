...
runner.unittest("PGC No Filter", function()
    local expResults = {
    ...
    local dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", radius=0, with_flags=true, catalog=contents, sort_by_index=true}))
...
runner.unittest("PGC Temporal Filter for Range T0 to T1", function()
    ...
    local dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, t0=t0str, t1=t1str, sort_by_index=true}))
    local tbl, err = dem:sample(lon, lat, height)
    runner.assert(err == 0)
    runner.assert(tbl ~= nil)
    local sampleCnt = 0
...
runner.unittest("PGC Temporal Filter Only T0", function()
    ...
    local dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, t0=t0str, sort_by_index=true}))
    local tbl, err = dem:sample(lon, lat, height)
    runner.assert(err == 0)
    runner.assert(tbl ~= nil)
    local sampleCnt = 0
...
runner.unittest("PGC Temporal Filter Only T1", function()
    ...
    local dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, t1=t1str, sort_by_index=true}))
    local tbl, err = dem:sample(lon, lat, height)
    runner.assert(err == 0)
    runner.assert(tbl ~= nil)
    local sampleCnt = 0
...
runner.unittest("PGC Temporal Filter In DOY Range", function()
    -- Keep only raster from 1st May to 31st July any year
    local doy_range_str = "121:212"
    local expResults = {
        {117.67968750, 0x0, '/vsis3/pgc-opendata-dems/arcticdem/strips/s2s041/2m/n70w151/SETSM_s2s041_WV02_20210622_10300100C0877F00_10300100C0947A00_2m_lsf_seg1_dem.tif'},
        {116.25000000, 0x0, '/vsis3/pgc-opendata-dems/arcticdem/strips/s2s041/2m/n69w151/SETSM_s2s041_WV02_20190620_10300100954C9000_1030010093D59200_2m_lsf_seg1_dem.tif'},
        {111.71093750, 0x4, '/vsis3/pgc-opendata-dems/arcticdem/strips/s2s041/2m/n69w151/SETSM_s2s041_WV01_20170710_1020010063D34400_102001006311E100_2m_seg2_dem.tif'},
    ...
    local dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, doy_range=doy_range_str, sort_by_index=true}))
    local tbl, err = dem:sample(lon, lat, height)
    runner.assert(err == 0)
    runner.assert(tbl ~= nil)
    local sampleCnt = 0
...
runner.unittest("PGC Temporal Filter Out DOY Range", function()
    -- Filter out rasters from 1st May to 31st July any year
    local doy_range_str = "!121:212"
    local expResults = {
        {120.36718750, 0x0, '/vsis3/pgc-opendata-dems/arcticdem/strips/s2s041/2m/n69w150/SETSM_s2s041_WV02_20220313_10300100CF178100_10300100CF1C4700_2m_lsf_seg1_dem.tif'},
        {116.13281250, 0x0, '/vsis3/pgc-opendata-dems/arcticdem/strips/s2s041/2m/n70w150/SETSM_s2s041_WV01_20210327_10200100A69B7D00_10200100A9635700_2m_lsf_seg1_dem.tif'},
    ...
    local dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", radius=0, catalog=contents, with_flags=true, doy_range=doy_range_str, sort_by_index=true}))
    local tbl, err = dem:sample(lon, lat, height)
    runner.assert(err == 0)
    runner.assert(tbl ~= nil)
    local sampleCnt = 0
...
runner.unittest("PGC Temporal Filter Closest Time", function()
    -- For closest time return only one raster with time closest to give time
    local tstr = "2021:2:4:23:3:0"
    local expResults = {{116.13281250, 0x0, '/vsis3/pgc-opendata-dems/arcticdem/strips/s2s041/2m/n70w150/SETSM_s2s041_WV01_20210327_10200100A69B7D00_10200100A9635700_2m_lsf_seg1_dem.tif'}}
    local dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", radius=0, catalog=contents, with_flags=true, closest_time=tstr, sort_by_index=true}))
    local tbl, err = dem:sample(lon, lat, height)
    runner.assert(err == 0)
    runner.assert(tbl ~= nil)
    local sampleCnt = 0
...
runner.unittest("PGC Temporal Filter Override Closest Time", function()
    ...
    local expResults = {{121.41406250, 0x0, '/vsis3/pgc-opendata-dems/arcticdem/strips/s2s041/2m/n69w150/SETSM_s2s041_WV02_20160607_10300100578F9400_10300100587BCE00_2m_lsf_seg1_dem.tif'}}
    local dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", radius=0, catalog=contents, with_flags=true, closest_time=tstr, sort_by_index=true, use_poi_time=true}))
