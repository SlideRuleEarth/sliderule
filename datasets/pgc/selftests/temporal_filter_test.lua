...
dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, t0=t0str, sort_by_index=true}))
tbl, err = dem:sample(lon, lat, height)
runner.assert(err == 0)
runner.assert(tbl ~= nil)
...
dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, t1=t1str, sort_by_index=true}))
tbl, err = dem:sample(lon, lat, height)
runner.assert(err == 0)
runner.assert(tbl ~= nil)
...
dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, doy_range=doy_range_str, sort_by_index=true}))
tbl, err = dem:sample(lon, lat, height)
runner.assert(err == 0)
runner.assert(tbl ~= nil)
...
print(string.format("\n--------------------------------\nTest: %s Temporal Filter: doy_range=%s\n--------------------------------", demType, doy_range_str))
dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", radius=0, catalog=contents, with_flags=true, doy_range=doy_range_str, sort_by_index=true}))
tbl, err = dem:sample(lon, lat, height)
runner.assert(err == 0)
runner.assert(tbl ~= nil)
...
expResults = {{116.13281250, 0x0, '/vsis3/pgc-opendata-dems/arcticdem/strips/s2s041/2m/n70w150/SETSM_s2s041_WV01_20210327_10200100A69B7D00_10200100A9635700_2m_lsf_seg1_dem.tif'}}
...
dem = geo.raster(geo.parms({asset=demType, algorithm="NearestNeighbour", radius=0, catalog=contents, with_flags=true, closest_time=tstr, sort_by_index=true, use_poi_time=true}))
