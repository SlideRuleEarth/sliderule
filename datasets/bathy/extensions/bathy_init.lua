...
while sys.alive() and ( (not bathy_mask_complete) or
    ...
    bathy_mask_complete = sys.fileexists(bathy_mask_local) or aws.s3download(bucket, bathy_mask_remote, bathy_mask_local)
    ...
    water_ri_mask_complete = sys.fileexists(water_ri_mask_local) or aws.s3download(bucket, water_ri_mask_remote, water_ri_mask_local)
    ...
    for deg = 0,5 do
        for _,dir in ipairs({"THU", "TVU"}) do
            ...
            local uncertainty_lut_status = sys.fileexists(uncertainty_lut_local) or aws.s3download(bucket, uncertainty_lut_remote, uncertainty_lut_local)
