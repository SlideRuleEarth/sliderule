...
runner.unittest("REMA DEM Mosaic Correct Values", function()
    local dem = geo.raster(geo.parms({asset="rema-mosaic", algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, sort_by_index=true}))
    local sampleCnt = 0
    local tbl, err = dem:sample(lon, lat, height)
    runner.assert(err == 0, string.format("Point: (%.3f, %.3f) ======> FAILED to read", lon, lat), true)
...
runner.unittest("REMA DEM Strips Correct Values", function()
    ...
    local dem = geo.raster(geo.parms({asset="rema-strips", algorithm="NearestNeighbour", catalog=contents, radius=0, with_flags=true, sort_by_index=true}))
    local sampleCnt = 0
    local tbl, err = dem:sample(lon, lat, height)
    runner.assert(err == 0, string.format("Point: (%.3f, %.3f) ======> FAILED to read", lon, lat), true)
    for k, v in ipairs(tbl) do
        ...
        print(string.format("(%02d) %16.8fm  qmask: 0x%x %s", k, el, flags, fname))
        sampleCnt = sampleCnt + 1
        runner.assert(math.abs(el - expResultsStrips[k][1]) < sigma)
        runner.assert(flags == expResultsStrips[k][2])
